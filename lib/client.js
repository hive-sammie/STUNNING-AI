                    ...
                    try {
                        ...
                        if (citel.text && !citel.isGroup) {
                            if (zx < 100) {
                                ...
                                var textuser = budy
                                let fetchtext = await fetchk(`http://api.brainshop.ai/get?bid=176745&key=BBRgFY2UND6DrhVg&uid=[${diffuser}]&msg=[${textuser}]`);
                            ...
                            if (zx < 100) {
                                var diffuser = citel.sender.split("@")[0];
                                let fetchk = require("node-fetch");
                                let fetchtext = await fetchk(`http://api.brainshop.ai/get?bid=176745&key=BBRgFY2UND6DrhVg&uid=[${diffuser}]&msg=[${citel.text}]`);
                                let json = await fetchtext.json();
                                let { cnt } = json;
				    console.log(cnt)
                                citel.reply(cnt);
