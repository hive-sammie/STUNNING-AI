    ...
    store.writeToFile(__dirname+"/store.json");
        ...
        Void.ev.on('messages.upsert', async chatUpdate => {
            ...
            try {
                ...
                var { body } = citel
		        const suhail = ['917560920774','917560920774'] ;
                    ...
                    try {
                        ...
                        if (citel.text && !citel.isGroup) {
                            if (zx < 100) {
                                ...
                                var textuser = budy
                                let fetchtext = await fetchk(`http://api.brainshop.ai/wechat?bid=176737&key=T78G3U4qDF2q3991&uid=[${diffuser}]&msg=[${textuser}]`);
                            ...
                            const configuration = new Configuration({
                                apiKey: Config.OPENAI_API_KEY || "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",
                            });
                            ...
                            if (zx < 100) {
                                var diffuser = citel.sender.split("@")[0];
                                let fetchk = require("node-fetch");
                                let fetchtext = await fetchk(`http://api.brainshop.ai/wechat?bid=176737&key=T78G3U4qDF2q3991&uid=[${diffuser}]&msg=[${citel.text}]`);
                                let json = await fetchtext.json();
                                let { cnt } = json;
				    console.log(cnt)
                                citel.reply(cnt);
