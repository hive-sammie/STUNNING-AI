...
Version: 0.0.6`, citel);
 ...
 cmd({
	...
	if(!users) return citel.reply('Please give me user.')
        const balance  = await eco.balance(users, "khadherinc")
        await eco.deduct(users, "khadherinc", balance.wallet);
 ...
 await citel.reply('*What are you trying to do📉*.')
    ...
    async(Void, citel, text,{ isCreator }) => {
      ...
      //  let users = citel.mentionedJid ? citel.mentionedJid[0] : citel.msg.contextInfo.participant || false;
        ...
        let d = parseInt(text)
        const deposit = await eco.deposit(citel.sender, "khadherinc", d);
        const balance = await eco.balance(citel.sender, "khadherinc")
...
Version: 0.0.6`, citel);
    ...
    async(Void, citel, text,{ isCreator }) => {
        ...
        let mongoschemas = zerogroup.economy || "false";
        if (mongoschemas == "false") return citel.reply("*🚦Economy* is not active in current group.");
        const balance = await eco.balance(citel.sender, "khadherinc"); //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
        let buttons = [{
...
async(Void, citel, text,{ isCreator }) => {
    ...
    if (mongoschemas == "false") return citel.reply("*🚦Economy* is not active in current group.");
    const kg = 100
            const balance1  = await eco.balance(citel.sender, "khadherinc")
    ...
    if(lol){
        ...
        const give2 = await eco.give(citel.sender, "khadherinc", deduff*2);
...
Version: 0.0.6`, citel);
    ...
    const deduct1 = await eco.deduct(citel.sender, "khadherinc", deduff);
    let st = `\n🎰 Slot Machine Result\n     ${i}\n\n      ${j}\n\n      ${k}\n\nNot Jacpot📉 but lost `
            let str = st.replace(/1/g, `🔴`).replace(/2/g, `🔵`).replace(/3/g, `🟣`).replace(/4/g, `🟢`).replace(/5/g, `🟡`).replace(/6/g, `⚪️`).replace(/7/g, `⚫️`).replace(/:/g, `    `)
    let buttons = [{
        buttonId: `${prefix}slot`,
